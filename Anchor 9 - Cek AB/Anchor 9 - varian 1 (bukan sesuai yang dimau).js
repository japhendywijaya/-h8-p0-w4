  ...
  for(x = 0; x < indexCharA.length; x++)
    {
      for(var y = 0; y < indexCharB.length; y++)
        {
          if (  (indexCharA[x] < indexCharB[y] && indexCharB[y] < indexCharA[x+1]) ||
                    (x=== indexCharA.length-1 && indexCharA[x] < indexCharB[y])    )
            {
              var tempPairAB = []
  ...
  for(x = 0; x < indexCharB.length; x++)
    {
      for(var y = 0; y < indexCharA.length; y++)
        {
          if (  (indexCharB[x] < indexCharA[y] && indexCharA[y] < indexCharB[x+1]) ||
                   (x=== indexCharB.length-1 && indexCharB[x] < indexCharA[y])    )
            {
              var tempPairBA = []
...
//  kesimpulannya, semua ketemuan a dan b dihitung